  ...
  if (isset($_POST['delete_schedule_permanently']) || isset($_POST['delete_schedule_temporarily'])) {
    ...
    if ($success) {
      $_SESSION['message'] = $is_permanent ?
        "Berhasil menghapus $processed jadwal secara permanen." :
        "Berhasil menghapus $processed jadwal sementara untuk pekan ini.";
      $_SESSION['alert_class'] = "alert-success";
    } else {
      $_SESSION['message'] = "Berhasil memproses $processed jadwal, namun gagal memproses $failed jadwal.";
...
<body>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      <?php if (isset($_SESSION['hidden_buttons']) && !empty($_SESSION['hidden_buttons'])): ?>
        var hiddenButtons = <?php echo json_encode($_SESSION['hidden_buttons']); ?>;
        hiddenButtons.forEach(function (buttonData) {
          hideKosongTemporaryButtonOnSuccess(buttonData.hari, buttonData.jam, buttonData.matkul, buttonData.dosenId, buttonData.kelas);
        });
        <?php unset($_SESSION['hidden_buttons']); ?>
      <?php endif; ?>
    });

    function hideKosongTemporaryButtonOnSuccess(hari, jam, matkul, dosenId, kelas) {
