...
function insert_schedule($db, $hari, $course, $dosen, $classroom, $kelas, $time_slots, $start_index, $end_index, $is_temporary = 0, $end_date = NULL)
{
    $message = "";
    $alert_class = "";

    // Check if all required parameters are set and valid
    if (empty($hari) || empty($course) || empty($dosen) || empty($classroom) || empty($kelas) || $start_index === false || $end_index === false || $start_index > $end_index) {
        return ["Error: Invalid input data", "alert-danger"];
    }

    $stmt = $db->prepare("INSERT INTO jadwal_kuliah (hari, matkul, dosen, classroom, kelas, jam, is_temporary, end_date)
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    for ($i = $start_index; $i <= $end_index; $i++) {
        ...
        $stmt->bind_param("ssssssis", $hari, $course, $dosen, $classroom, $kelas, $current_time, $is_temporary, $end_date);
...
function fetch_schedules($db, $class)
{
    $schedules = [];
    $sql = "SELECT * FROM jadwal_kuliah WHERE kelas = ?";
    $stmt = $db->prepare($sql);
...
function update_or_insert_schedule($db, $hari, $matkul_id, $dosen, $classroom, $kelas, $jam_mulai, $jam_selesai, $time_slots, $is_temporary = 0, $end_date = NULL, $old_hari = NULL, $old_jam_mulai = NULL, $old_jam_selesai = NULL)
{
    $message = "";
    $alert_class = "";
    ...
    for ($i = $start_index; $i <= $end_index; $i++) {
        ...
        if (isset($existing_schedules[$current_time])) {
            $sql = "UPDATE jadwal_kuliah SET matkul = ?, dosen = ?, classroom = ?, is_temporary = ?, end_date = ? WHERE id = ?";
            $stmt = $db->prepare($sql);
            $stmt->bind_param("sssssi", $course, $dosen, $classroom, $is_temporary, $end_date, $existing_schedules[$current_time]);
            unset($existing_schedules[$current_time]);
        } else {
            $sql = "INSERT INTO jadwal_kuliah (hari, matkul, dosen, classroom, kelas, jam, is_temporary, end_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
            $stmt->bind_param("ssssssis", $hari, $course, $dosen, $classroom, $kelas, $current_time, $is_temporary, $end_date);
        }
...
function fetch_schedule_by_email($db, $user_email)
{
    $time_slots = get_time_slots_for_viewing();
    $days = ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'];

    // Prepare the statement to fetch the schedule based on user email
    $stmt = $db->prepare('SELECT jk.* FROM jadwal_kuliah jk JOIN dosen_profiles dp ON jk.dosen = dp.nama WHERE dp.email = ?');
...
function fetch_all_slots($db)
{
    ...
    foreach ($days as $day) {
        foreach ($time_slots as $slot) {
            $sql = "SELECT * FROM jadwal_kuliah WHERE hari = ? AND jam = ? AND (is_temporary = 0 OR (is_temporary = 1 AND end_date >= CURDATE()))";
            $stmt = $db->prepare($sql);
...
function getEmailByDosenName($db, $nama_dosen)
{
    $query = "SELECT email FROM dosen_profiles WHERE nama = ?";
    $stmt = $db->prepare($query);
    $stmt->bind_param("s", $nama_dosen);
    $stmt->execute();
    $result = $stmt->get_result();
    $dosen = $result->fetch_assoc();
    return $dosen ? $dosen['email'] : null;
}
...
function processCourseFormByAdmin($db)
{
    ...
    if (isset($_POST['kode'], $_POST['nama'], $_POST['deskripsi'], $_POST['dosen'])) {
        // Escape special characters to prevent XSS
        $kode = htmlspecialchars($_POST['kode']);
        $nama = htmlspecialchars($_POST['nama']);
        $deskripsi = htmlspecialchars($_POST['deskripsi']);
        ...
        // Get email dosen from dosen name
        $email_dosen = getEmailByDosenName($db, $dosen);

        // Prepare SQL statement without the kontrak_kuliah field
        $sql = "INSERT INTO mata_kuliah (kode, nama, deskripsi, dosen, email_dosen, status) VALUES (?, ?, ?, ?, ?, 'Pending')";
        $stmt = $db->prepare($sql);
        $stmt->bind_param("sssss", $kode, $nama, $deskripsi, $dosen, $email_dosen);
